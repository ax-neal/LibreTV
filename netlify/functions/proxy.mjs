...
try {
  ...
  if (agentsJsonString) {
    ...
    if (Array.isArray(parsedAgents) && parsedAgents.length > 0) {
      ...
      console.log(
        `[Proxy Log Netlify] Loaded ${USER_AGENTS.length} user agents from environment variable.`
      );
    } else {
      console.warn(
        "[Proxy Log Netlify] USER_AGENTS_JSON environment variable is not a valid non-empty array, using default."
      );
    }
  } else {
    console.log(
      "[Proxy Log Netlify] USER_AGENTS_JSON environment variable not set, using default user agents."
    );
  }
} catch (e) {
  console.error(
    `[Proxy Log Netlify] Error parsing USER_AGENTS_JSON environment variable: ${e.message}. Using default user agents.`
  );
}
  ...
  try {
    ...
    if (decodedUrl.match(/^https?:\/\/.+/i)) {
      ...
      if (encodedPath.match(/^https?:\/\/.+/i)) {
        logDebug(
          `Warning: Path was not encoded but looks like URL: ${encodedPath}`
        );
        return encodedPath;
      }
      return null;
...
function resolveUrl(baseUrl, relativeUrl) {
  ...
  try {
    ...
    logDebug(
      `URL resolution failed: base="${baseUrl}", relative="${relativeUrl}". Error: ${e.message}`
    );
...
async function fetchContentWithType(targetUrl, requestHeaders) {
  ...
  logDebug(
    `Fetching target: ${targetUrl} with headers: ${JSON.stringify(headers)}`
  );
  try {
    ...
    if (!response.ok) {
      ...
      logDebug(
        `Fetch failed: ${response.status} ${response.statusText} - ${targetUrl}`
      );
      const err = new Error(
        `HTTP error ${response.status}: ${
          response.statusText
        }. URL: ${targetUrl}. Body: ${errorBody.substring(0, 200)}`
      );
    ...
    logDebug(
      `Fetch success: ${targetUrl}, Content-Type: ${contentType}, Length: ${content.length}`
    );
    return { content, contentType, responseHeaders: response.headers };
  } catch (error) {
    logDebug(`Fetch exception for ${targetUrl}: ${error.message}`);
    throw new Error(
      `Failed to fetch target URL ${targetUrl}: ${error.message}`
    );
  }
}
...
function processKeyLine(line, baseUrl) {
  return line.replace(/URI="([^"]+)"/, (match, uri) => {
    ...
    logDebug(
      `Processing KEY URI: Original='${uri}', Absolute='${absoluteUri}'`
    );
...
function processMapLine(line, baseUrl) {
  return line.replace(/URI="([^"]+)"/, (match, uri) => {
    const absoluteUri = resolveUrl(baseUrl, uri);
    logDebug(
      `Processing MAP URI: Original='${uri}', Absolute='${absoluteUri}'`
    );
    return `URI="${rewriteUrlToProxy(absoluteUri)}"`;
  });
}
function processMediaPlaylist(url, content) {
  ...
  if (!baseUrl) {
    logDebug(
      `Could not determine base URL for media playlist: ${url}. Cannot process relative paths.`
    );
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line.startsWith("#")) {
      ...
      logDebug(
        `Rewriting media segment: Original='${line}', Resolved='${absoluteUrl}'`
      );
...
async function processM3u8Content(targetUrl, content, recursionDepth = 0) {
  if (
    ...
    logDebug(
      `Detected master playlist: ${targetUrl} (Depth: ${recursionDepth})`
    );
...
async function processMasterPlaylist(url, content, recursionDepth) {
  if (recursionDepth > MAX_RECURSION) {
    throw new Error(
      `Max recursion depth (${MAX_RECURSION}) exceeded for master playlist: ${url}`
    );
  }
  const baseUrl = getBaseUrl(url);
  const lines = content.split("\n");
  ...
  if (!bestVariantUrl) {
    logDebug(
      `No valid sub-playlist URI found in master: ${url}. Processing as media playlist.`
    );
    return processMediaPlaylist(url, content);
  }
  logDebug(
    `Selected sub-playlist (Bandwidth: ${highestBandwidth}): ${bestVariantUrl}`
  );
  ...
  if (!isM3u8Content(variantContent, variantContentType)) {
    logDebug(
      `Fetched sub-playlist ${bestVariantUrl} is not M3U8 (Type: ${variantContentType}). Treating as media playlist.`
    );
...
export const handler = async (event, context) => {
  ...
  if (!targetUrl) {
    ...
    return {
      ...
      body: JSON.stringify({
        success: false,
        error: "Invalid proxy request path. Could not extract target URL.",
      }),
    };
  }
  ...
  try {
    ...
    if (!isValidAuth) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "Forbidden: Invalid auth credentials.",
        }),
      };
    }
    ...
    const { content, contentType, responseHeaders } =
      await fetchContentWithType(targetUrl, event.headers);
    ...
    if (isM3u8Content(content, contentType)) {
      ...
      logDebug(
        `Returning non-M3U8 content directly: ${targetUrl}, Type: ${contentType}`
      );
