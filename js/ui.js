...
function showToast(message, type = "error") {
  ...
  if (!toast) {
    ...
    toast.id = "toast";
    toast.className =
      "fixed top-4 left-1/2 -translate-x-1/2 px-6 py-3 rounded-lg shadow-lg transform transition-all duration-300 z-50 opacity-0";
...
function renderSearchHistory() {
  ...
  history.forEach((item) => {
    ...
    deleteButton.className =
      "pl-1 text-gray-500 hover:text-red-500 transition-colors";
    ...
    .map((item) => {
      ...
      if (
        ...
        const percent = Math.round(
          (item.playbackPosition / item.duration) * 100
        );
      ...
      return `
            <div class="history-item cursor-pointer relative group" onclick="playFromHistory('${
              item.url
            }', '${safeTitle}', ${item.episodeIndex || 0}, ${
        item.playbackPosition || 0
                ...
                <div class="history-info">
                    ...
                    <div class="history-meta">
                        ...
                        ${
                          episodeText
                            ? '<span class="history-separator mx-1">·</span>'
                            : ""
                        }
                    ...
                    ${progressHtml}
                    <div class="history-time">${formatTimestamp(
                      item.timestamp
                    )}</div>
                </div>
            </div>
  ...
  try {
    ...
    if (historyItem && historyItem.vod_id && historyItem.sourceName) {
      ...
      try {
        ...
        const apiUrl = `/api/detail?id=${encodeURIComponent(
          historyItem.vod_id
        ...
        if (
          ...
          videoDetails.episodes.length > 0
        ) {
          ...
          if (newEpisodeCount > oldEpisodeCount) {
            showToast(
              `已同步最新剧集列表 (${newEpisodeCount}集，新增${
                newEpisodeCount - oldEpisodeCount
              }集)`,
              "success"
            );
    ...
    let currentPath;
    if (
      window.location.pathname.startsWith("/player.html") ||
      window.location.pathname.startsWith("/watch.html")
    ) {
    ...
    const sourceNameForUrl = historyItem
      ? historyItem.sourceName
      : new URLSearchParams(new URL(url, window.location.origin).search).get(
          "source"
        );
    ...
    if (url.includes("player.html") || url.includes("watch.html")) {
      ...
      try {
        ...
        playerUrl = `player.html?url=${encodeURIComponent(
          realVideoUrl
        )}&title=${encodeURIComponent(
          title
        )}&index=${episodeIndex}&position=${Math.floor(
          playbackPosition || 0
        )}&returnUrl=${encodeURIComponent(currentPath)}`;
        if (sourceNameForUrl)
          playerUrl += `&source=${encodeURIComponent(sourceNameForUrl)}`;
        if (sourceCodeForUrl)
          playerUrl += `&source_code=${encodeURIComponent(sourceCodeForUrl)}`;
      ...
      playUrl.searchParams.set(
        "position",
        Math.floor(playbackPosition || 0).toString()
      );
      playUrl.searchParams.set("returnUrl", encodeURIComponent(currentPath));
      if (sourceNameForUrl)
        playUrl.searchParams.set("source", sourceNameForUrl);
      if (sourceCodeForUrl)
        playUrl.searchParams.set("source_code", sourceCodeForUrl);
    ...
    const simpleUrl = `player.html?url=${encodeURIComponent(
      url
    )}&title=${encodeURIComponent(title)}&index=${episodeIndex}`;
  ...
  try {
    ...
    if (!videoInfo.showIdentifier) {
      if (videoInfo.sourceName && videoInfo.vod_id) {
        ...
        videoInfo.showIdentifier =
          videoInfo.episodes && videoInfo.episodes.length > 0
            ? videoInfo.episodes[0]
    ...
    if (existingIndex !== -1) {
      ...
      existingItem.directVideoUrl =
        videoInfo.directVideoUrl || existingItem.directVideoUrl;
      ...
      if (
        videoInfo.episodes &&
        Array.isArray(videoInfo.episodes) &&
        videoInfo.episodes.length > 0
      ) {
        if (
...
document.addEventListener("DOMContentLoaded", function () {
  document.addEventListener("click", function (e) {
    ...
    const historyButton = document.querySelector(
      'button[onclick="toggleHistory(event)"]'
    );
...
function clearLocalStorage() {
  ...
  // 添加事件监听器 - 确定按钮
  document
    .getElementById("confirmBoxModal")
    .addEventListener("click", function () {
      // 清除所有localStorage数据
      localStorage.clear();
