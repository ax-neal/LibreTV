  ...
  try {
    if (url.pathname === "/api/search") {
      ...
      const apiUrl = customApi
        ? `${customApi}${API_CONFIG.search.path}${encodeURIComponent(
            searchQuery
          )}`
      ...
      try {
        ...
        data.list.forEach((item) => {
          item.source_name =
            source === "custom" ? "自定义源" : API_SITES[source].name;
    ...
    if (url.pathname === "/api/detail") {
      ...
      if (
        sourceCode === "custom" &&
        url.searchParams.get("useDetail") === "true"
      ) {
      ...
      try {
        ...
        return JSON.stringify({
          ...
          videoInfo: {
            ...
            // 添加源信息
            source_name:
              sourceCode === "custom" ? "自定义源" : API_SITES[sourceCode].name,
...
async function handleAggregatedSearch(searchQuery) {
  ...
  const searchPromises = availableSources.map(async (source) => {
    try {
      const apiUrl = `${API_SITES[source].api}${
        API_CONFIG.search.path
      }${encodeURIComponent(searchQuery)}`;
...
async function handleMultipleCustomSearch(searchQuery, customApiUrls) {
  ...
  const searchPromises = apiUrls.map(async (apiUrl, index) => {
    try {
      const fullUrl = `${apiUrl}${API_CONFIG.search.path}${encodeURIComponent(
        searchQuery
      )}`;
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`自定义API ${index + 1} 搜索超时`)),
          8000
        )
      );
...
async function testSiteAvailability(apiUrl) {
  try {
    ...
    const response = await fetch(
      "/api/search?wd=test&customApi=" + encodeURIComponent(apiUrl),
      {
        // 添加超时
