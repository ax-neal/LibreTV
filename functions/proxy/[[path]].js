...
export async function onRequest(context) {
  ...
  async function validateAuth(request, env) {
    ...
    try {
      ...
      const serverPasswordHash = hashArray
    ...
    try {
      ...
      if (!decodedUrl.match(/^https?:\/\//i)) {
        ...
        if (encodedUrl.match(/^https?:\/\//i)) {
          ...
          logDebug(
            `Warning: Path was not encoded but looks like URL: ${decodedUrl}`
          );
        } else {
  ...
  function resolveUrl(baseUrl, relativeUrl) {
    ...
    try {
      ...
      logDebug(
        `解析 URL 失败: baseUrl=${baseUrl}, relativeUrl=${relativeUrl}, error=${e.message}`
      );
      // 简单的备用方法
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `HTTP error ${response.status}: ${
            response.statusText
          }. URL: ${targetUrl}. Body: ${errorBody.substring(0, 150)}`
        );
      }
  ...
  async function processM3u8Content(
    ...
    if (
      ...
      return await processMasterPlaylist(
        targetUrl,
        content,
        recursionDepth,
        env
  ...
  async function processMasterPlaylist(url, content, recursionDepth, env) {
    ...
    if (!bestVariantUrl) {
      logDebug(
        `主列表中未找到 BANDWIDTH 或 STREAM-INF，尝试查找第一个子列表引用: ${url}`
      );
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        if (
          line &&
          !line.startsWith("#") &&
          (line.endsWith(".m3u8") || line.includes(".m3u8?"))
        ) {
          // 修复：检查是否包含 .m3u8?
    ...
    if (!bestVariantUrl) {
      logDebug(
        `在主列表 ${url} 中未找到任何有效的子播放列表 URL。可能格式有问题或仅包含音频/字幕。将尝试按媒体列表处理原始内容。`
      );
    ...
    if (!isM3u8Content(variantContent, variantContentType)) {
      logDebug(
        `获取到的子列表 ${bestVariantUrl} 不是 M3U8 内容 (类型: ${variantContentType})。可能直接是媒体文件，返回原始内容。`
      );
    ...
    const processedVariant = await processM3u8Content(
      bestVariantUrl,
      variantContent,
      recursionDepth + 1,
      env
    ...
    if (kvNamespace) {
      try {
        ...
        waitUntil(
          kvNamespace.put(cacheKey, processedVariant, {
  ...
  try {
      ...
      try {
        ...
        if (cachedDataJson) {
          ...
          if (isM3u8Content(content, contentType)) {
            ...
            const processedM3u8 = await processM3u8Content(
              targetUrl,
              content,
              0,
              env
    ...
    if (kvNamespace) {
      try {
        ...
        // 注意 KV 写入限制
        waitUntil(
          kvNamespace.put(cacheKey, JSON.stringify(cacheValue), {
            expirationTtl: CACHE_TTL,
          })
        );
